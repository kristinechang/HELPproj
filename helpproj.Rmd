...
#variables tried (and not interested): age, gender, race (ask hardin about this one tomorrow morning), M16, E10B1_R, E15C2, G1B_30
#variables interested: group (by default), education, anysubstatus, abuse2, alcohol, F1I 
...
coxph(Surv(dayslink,linkstatus) ~ e15c2, data = help) %>% tidy()
coxph(Surv(dayslink,linkstatus) ~ 1, data = help) %>% glance()
...
#E15C2 is not significant (even though G-stat is significant; look at tidy()pval and logCI on KM curves)

help %>%
  group_by(e15c2) %>%
  summarize(n())

#seems like there's not that many NAs
```
...
help_surv <- survfit(Surv(dayslink, linkstatus) ~ g1b_30, data=help)
ggsurvplot(help_surv, conf.type="log", censor=F, conf.int=T) +
  ggtitle("HELP data EDA")

coxph(Surv(dayslink,linkstatus) ~ g1b_30, data = help) %>% glance()
coxph(Surv(dayslink,linkstatus) ~ g1b_30, data = help) %>% tidy()
coxph(Surv(dayslink,linkstatus) ~ 1, data = help) %>% glance()

#G-statistic
G11 <- 2*(-177.9322-(-718.0145))
1-pchisq(G11,1)

#G1B_30 is not significant, even though G-stat gives a significant result; look at tidy()p-val and logCI on KM curves
